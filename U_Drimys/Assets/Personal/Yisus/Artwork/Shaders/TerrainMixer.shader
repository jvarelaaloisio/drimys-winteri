...
Shader "TerrainMixer"
{
	...
	SubShader
	{
		...
		float2 voronoihash2_g232( float2 p )
		{
		...
		float voronoi2_g232( float2 v, float time, inout float2 id, inout float2 mr, float smoothness )
		{
			float2 n = floor( v );
			float2 f = frac( v );
			float F1 = 8.0;
			...
			for ( int j = -1; j <= 1; j++ )
			{
				for ( int i = -1; i <= 1; i++ )
			 	{
			 		float2 g = float2( i, j );
			 		float2 o = voronoihash2_g232( n + g );
		...
		float2 voronoihash2_g234( float2 p )
		{
		...
		float voronoi2_g234( float2 v, float time, inout float2 id, inout float2 mr, float smoothness )
		{
			float2 n = floor( v );
			float2 f = frac( v );
			float F1 = 8.0;
			...
			for ( int j = -1; j <= 1; j++ )
			{
				for ( int i = -1; i <= 1; i++ )
			 	{
			 		float2 g = float2( i, j );
			 		float2 o = voronoihash2_g234( n + g );
					o = ( sin( time + o * 6.2831 ) * 0.5 + 0.5 ); float2 r = f - g - o;
					float d = 0.5 * dot( r, r );
			 		if( d<F1 ) {
			 			F2 = F1;
		...
		float2 voronoihash2_g233( float2 p )
		{
		...
		float voronoi2_g233( float2 v, float time, inout float2 id, inout float2 mr, float smoothness )
		{
			float2 n = floor( v );
			float2 f = frac( v );
			float F1 = 8.0;
			...
			for ( int j = -1; j <= 1; j++ )
			{
				for ( int i = -1; i <= 1; i++ )
			 	{
			 		float2 g = float2( i, j );
			 		float2 o = voronoihash2_g233( n + g );
					o = ( sin( time + o * 6.2831 ) * 0.5 + 0.5 ); float2 r = f - g - o;
					float d = 0.5 * dot( r, r );
			 		if( d<F1 ) {
			 			F2 = F1;
		...
		float2 voronoihash2_g228( float2 p )
		{
		...
		float voronoi2_g228( float2 v, float time, inout float2 id, inout float2 mr, float smoothness )
		{
			float2 n = floor( v );
			float2 f = frac( v );
			float F1 = 8.0;
			...
			for ( int j = -1; j <= 1; j++ )
			{
				for ( int i = -1; i <= 1; i++ )
			 	{
			 		float2 g = float2( i, j );
			 		float2 o = voronoihash2_g228( n + g );
					o = ( sin( time + o * 6.2831 ) * 0.5 + 0.5 ); float2 r = f - g - o;
					float d = 0.5 * dot( r, r );
			 		if( d<F1 ) {
			 			F2 = F1;
		...
		float2 voronoihash2_g230( float2 p )
		{
		...
		float voronoi2_g230( float2 v, float time, inout float2 id, inout float2 mr, float smoothness )
		{
			float2 n = floor( v );
			float2 f = frac( v );
			float F1 = 8.0;
			...
			for ( int j = -1; j <= 1; j++ )
			{
				for ( int i = -1; i <= 1; i++ )
			 	{
			 		float2 g = float2( i, j );
			 		float2 o = voronoihash2_g230( n + g );
					o = ( sin( time + o * 6.2831 ) * 0.5 + 0.5 ); float2 r = f - g - o;
					float d = 0.5 * dot( r, r );
			 		if( d<F1 ) {
			 			F2 = F1;
		...
		float2 voronoihash2_g229( float2 p )
		{
		...
		float voronoi2_g229( float2 v, float time, inout float2 id, inout float2 mr, float smoothness )
		{
			float2 n = floor( v );
			float2 f = frac( v );
			float F1 = 8.0;
			...
			for ( int j = -1; j <= 1; j++ )
			{
				for ( int i = -1; i <= 1; i++ )
			 	{
			 		float2 g = float2( i, j );
			 		float2 o = voronoihash2_g229( n + g );
					o = ( sin( time + o * 6.2831 ) * 0.5 + 0.5 ); float2 r = f - g - o;
					float d = 0.5 * dot( r, r );
			 		if( d<F1 ) {
			 			F2 = F1;
		...
		void vertexDataFunc( inout appdata_full v )
		{
			...
			float3 temp_output_20_0_g231 = VertexColor_var61.rgb;
			float3 break21_g231 = temp_output_20_0_g231;
			float temp_output_9_0_g232 = break21_g231.x;
			float NoiseIntensityval186 = _Noseintensityval;
			float temp_output_46_0_g231 = NoiseIntensityval186;
			float Noisescaleval190 = _Noisescaleval;
			float temp_output_47_0_g231 = Noisescaleval190;
			float temp_output_11_0_g232 = temp_output_47_0_g231;
			float time2_g232 = 0.0;
			float2 coords2_g232 = v.texcoord.xy * temp_output_11_0_g232;
			float2 id2_g232 = 0;
			float2 uv2_g232 = 0;
			float voroi2_g232 = voronoi2_g232( coords2_g232, time2_g232, id2_g232, uv2_g232, 0 );
			float simplePerlin2D12_g232 = snoise( v.texcoord.xy*temp_output_11_0_g232 );
			simplePerlin2D12_g232 = simplePerlin2D12_g232*0.5 + 0.5;
			float temp_output_53_0_g231 = _WhitNoiseLerpTex4;
			float lerpResult52_g231 = lerp( break21_g231.x , ( (0.0 + (temp_output_9_0_g232 - 0.0) * (temp_output_46_0_g231 - 0.0) / (1.0 - 0.0)) * ( temp_output_9_0_g232 + ( voroi2_g232 + simplePerlin2D12_g232 ) ) ) , temp_output_53_0_g231);
			float temp_output_3_0_g231 = saturate( lerpResult52_g231 );
			float temp_output_9_0_g234 = break21_g231.y;
			float temp_output_11_0_g234 = temp_output_47_0_g231;
			float time2_g234 = 0.0;
			float2 coords2_g234 = v.texcoord.xy * temp_output_11_0_g234;
			float2 id2_g234 = 0;
			float2 uv2_g234 = 0;
			float voroi2_g234 = voronoi2_g234( coords2_g234, time2_g234, id2_g234, uv2_g234, 0 );
			float simplePerlin2D12_g234 = snoise( v.texcoord.xy*temp_output_11_0_g234 );
			simplePerlin2D12_g234 = simplePerlin2D12_g234*0.5 + 0.5;
			float lerpResult56_g231 = lerp( break21_g231.y , ( (0.0 + (temp_output_9_0_g234 - 0.0) * (temp_output_46_0_g231 - 0.0) / (1.0 - 0.0)) * ( temp_output_9_0_g234 + ( voroi2_g234 + simplePerlin2D12_g234 ) ) ) , temp_output_53_0_g231);
			float temp_output_2_0_g231 = saturate( lerpResult56_g231 );
			float temp_output_9_0_g233 = break21_g231.z;
			float temp_output_11_0_g233 = temp_output_47_0_g231;
			float time2_g233 = 0.0;
			float2 coords2_g233 = v.texcoord.xy * temp_output_11_0_g233;
			float2 id2_g233 = 0;
			float2 uv2_g233 = 0;
			float voroi2_g233 = voronoi2_g233( coords2_g233, time2_g233, id2_g233, uv2_g233, 0 );
			float simplePerlin2D12_g233 = snoise( v.texcoord.xy*temp_output_11_0_g233 );
			simplePerlin2D12_g233 = simplePerlin2D12_g233*0.5 + 0.5;
			float lerpResult57_g231 = lerp( break21_g231.z , ( (0.0 + (temp_output_9_0_g233 - 0.0) * (temp_output_46_0_g231 - 0.0) / (1.0 - 0.0)) * ( temp_output_9_0_g233 + ( voroi2_g233 + simplePerlin2D12_g233 ) ) ) , temp_output_53_0_g231);
			float temp_output_4_0_g231 = saturate( lerpResult57_g231 );
			float3 ase_vertex3Pos = v.vertex.xyz;
			v.vertex.xyz += ( ( _Tesellationfactor * ( ( tex2Dlod( _BaseHeight, float4( UV171, 0, 0.0) ).r * ( 1.0 - saturate( ( temp_output_3_0_g231 + temp_output_2_0_g231 + temp_output_4_0_g231 ) ) ) ) + ( ( ( tex2Dlod( _Height_1, float4( UV171, 0, 0.0) ).r * temp_output_3_0_g231 ) + ( tex2Dlod( _Height_2, float4( UV171, 0, 0.0) ).r * temp_output_2_0_g231 ) ) + ( tex2Dlod( _Height_3, float4( UV171, 0, 0.0) ).r * temp_output_4_0_g231 ) ) ) ) * ase_vertex3Pos );
			v.vertex.w = 1;
		}
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float2 coords2_g240 = i.uv_texcoord * temp_output_11_0_g240;
			float2 id2_g240 = 0;
			float2 uv2_g240 = 0;
			float voroi2_g240 = voronoi2_g240( coords2_g240, time2_g240, id2_g240, uv2_g240, 0 );
			float simplePerlin2D12_g240 = snoise( i.uv_texcoord*temp_output_11_0_g240 );
			simplePerlin2D12_g240 = simplePerlin2D12_g240*0.5 + 0.5;
			float temp_output_53_0_g239 = _WhitNoiseLerpTex2;
			...
			float2 coords2_g242 = i.uv_texcoord * temp_output_11_0_g242;
			float2 id2_g242 = 0;
			float2 uv2_g242 = 0;
			float voroi2_g242 = voronoi2_g242( coords2_g242, time2_g242, id2_g242, uv2_g242, 0 );
			float simplePerlin2D12_g242 = snoise( i.uv_texcoord*temp_output_11_0_g242 );
			...
			float2 coords2_g241 = i.uv_texcoord * temp_output_11_0_g241;
			float2 id2_g241 = 0;
			float2 uv2_g241 = 0;
			float voroi2_g241 = voronoi2_g241( coords2_g241, time2_g241, id2_g241, uv2_g241, 0 );
			float simplePerlin2D12_g241 = snoise( i.uv_texcoord*temp_output_11_0_g241 );
			...
			o.Normal = ( ( UnpackNormal( tex2D( _BaseNormal, UV171 ) ) * ( 1.0 - saturate( ( temp_output_3_0_g239 + temp_output_2_0_g239 + temp_output_4_0_g239 ) ) ) ) + ( ( ( UnpackNormal( tex2D( _Normal_1, UV171 ) ) * temp_output_3_0_g239 ) + ( UnpackNormal( tex2D( _Normal_2, UV171 ) ) * temp_output_2_0_g239 ) ) + ( UnpackNormal( tex2D( _Normal_3, UV171 ) ) * temp_output_4_0_g239 ) ) );
			...
			float temp_output_53_0_g235 = _WhitNoiseLerpTex1;
			...
			o.Albedo = ( ( tex2D( _BaseBaseColor, UV171 ) * ( 1.0 - saturate( ( temp_output_3_0_g235 + temp_output_2_0_g235 + temp_output_4_0_g235 ) ) ) ) + ( ( ( tex2D( _Texture_1, UV171 ) * temp_output_3_0_g235 ) + ( tex2D( _Texture_2, UV171 ) * temp_output_2_0_g235 ) ) + ( tex2D( _Texture_3, UV171 ) * temp_output_4_0_g235 ) ) ).rgb;
			float4 color104 = IsGammaSpace() ? float4(0,0,0,0) : float4(0,0,0,0);
			o.Metallic = color104.r;
			float3 temp_output_20_0_g227 = VertexColor_var61.rgb;
			float3 break21_g227 = temp_output_20_0_g227;
			float temp_output_9_0_g228 = break21_g227.x;
			float temp_output_46_0_g227 = NoiseIntensityval186;
			float temp_output_47_0_g227 = Noisescaleval190;
			float temp_output_11_0_g228 = temp_output_47_0_g227;
			float time2_g228 = 0.0;
			float2 coords2_g228 = i.uv_texcoord * temp_output_11_0_g228;
			float2 id2_g228 = 0;
			float2 uv2_g228 = 0;
			float voroi2_g228 = voronoi2_g228( coords2_g228, time2_g228, id2_g228, uv2_g228, 0 );
			float simplePerlin2D12_g228 = snoise( i.uv_texcoord*temp_output_11_0_g228 );
			simplePerlin2D12_g228 = simplePerlin2D12_g228*0.5 + 0.5;
			float temp_output_53_0_g227 = _WhitNoiseLerpTex3;
			float lerpResult52_g227 = lerp( break21_g227.x , ( (0.0 + (temp_output_9_0_g228 - 0.0) * (temp_output_46_0_g227 - 0.0) / (1.0 - 0.0)) * ( temp_output_9_0_g228 + ( voroi2_g228 + simplePerlin2D12_g228 ) ) ) , temp_output_53_0_g227);
			float temp_output_3_0_g227 = saturate( lerpResult52_g227 );
			float temp_output_9_0_g230 = break21_g227.y;
			float temp_output_11_0_g230 = temp_output_47_0_g227;
			float time2_g230 = 0.0;
			float2 coords2_g230 = i.uv_texcoord * temp_output_11_0_g230;
			float2 id2_g230 = 0;
			float2 uv2_g230 = 0;
			float voroi2_g230 = voronoi2_g230( coords2_g230, time2_g230, id2_g230, uv2_g230, 0 );
			float simplePerlin2D12_g230 = snoise( i.uv_texcoord*temp_output_11_0_g230 );
			simplePerlin2D12_g230 = simplePerlin2D12_g230*0.5 + 0.5;
			float lerpResult56_g227 = lerp( break21_g227.y , ( (0.0 + (temp_output_9_0_g230 - 0.0) * (temp_output_46_0_g227 - 0.0) / (1.0 - 0.0)) * ( temp_output_9_0_g230 + ( voroi2_g230 + simplePerlin2D12_g230 ) ) ) , temp_output_53_0_g227);
			float temp_output_2_0_g227 = saturate( lerpResult56_g227 );
			float temp_output_9_0_g229 = break21_g227.z;
			float temp_output_11_0_g229 = temp_output_47_0_g227;
			float time2_g229 = 0.0;
			float2 coords2_g229 = i.uv_texcoord * temp_output_11_0_g229;
			float2 id2_g229 = 0;
			float2 uv2_g229 = 0;
			float voroi2_g229 = voronoi2_g229( coords2_g229, time2_g229, id2_g229, uv2_g229, 0 );
			float simplePerlin2D12_g229 = snoise( i.uv_texcoord*temp_output_11_0_g229 );
			simplePerlin2D12_g229 = simplePerlin2D12_g229*0.5 + 0.5;
			float lerpResult57_g227 = lerp( break21_g227.z , ( (0.0 + (temp_output_9_0_g229 - 0.0) * (temp_output_46_0_g227 - 0.0) / (1.0 - 0.0)) * ( temp_output_9_0_g229 + ( voroi2_g229 + simplePerlin2D12_g229 ) ) ) , temp_output_53_0_g227);
			float temp_output_4_0_g227 = saturate( lerpResult57_g227 );
			float3 temp_cast_6 = (( 1.0 - ( ( tex2D( _BaseRoughness, UV171 ).r * ( 1.0 - saturate( ( temp_output_3_0_g227 + temp_output_2_0_g227 + temp_output_4_0_g227 ) ) ) ) + ( ( ( tex2D( _Roughness_1, UV171 ).r * temp_output_3_0_g227 ) + ( tex2D( _Roughness_2, UV171 ).r * temp_output_2_0_g227 ) ) + ( tex2D( _Roughness_3, UV171 ).r * temp_output_4_0_g227 ) ) ) )).xxx;
...
/*ASEBEGIN
Version=18500
1276;936;2004;1163;3048.552;-953.3364;2.463734;True;False
Node;AmplifyShaderEditor.RangedFloatNode;175;-2043.17,-771.9156;Inherit;False;Property;_Tiling;Tiling;22;0;Create;True;0;0;False;0;False;0;70;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;108;-1863.991,286.2975;Inherit;False;Property;_Noseintensityval;Nose intensity val;16;0;Create;True;0;0;False;0;False;0;0.6;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;109;-1865.441,368.5565;Inherit;False;Property;_Noisescaleval;Noise scale val;17;0;Create;True;0;0;False;0;False;0;500;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.FunctionNode;183;-417.1389,2294;Inherit;False;TextureBlend;-1;;231;24e89ccf6f531df44bc7fec5ad48af13;0;8;53;FLOAT;0;False;46;FLOAT;0;False;47;FLOAT;0;False;22;FLOAT;0;False;23;FLOAT;0;False;26;FLOAT;0;False;27;FLOAT;0;False;20;FLOAT3;0,0,0;False;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;166;-457.8563,1276.284;Inherit;False;TextureBlend;-1;;227;24e89ccf6f531df44bc7fec5ad48af13;0;8;53;FLOAT;0;False;46;FLOAT;0;False;47;FLOAT;0;False;22;FLOAT;0;False;23;FLOAT;0;False;26;FLOAT;0;False;27;FLOAT;0;False;20;FLOAT3;0,0,0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;128;-1134.277,101.5252;Inherit;False;Property;_WhitNoiseLerpTex2;Whit Noise Lerp Tex 2;19;0;Create;True;0;0;False;0;False;0;0;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;62;-1056,0;Inherit;False;61;VertexColor_var;1;0;OBJECT;;False;1;COLOR;0
Node;AmplifyShaderEditor.RangedFloatNode;121;-1141.558,-874.7564;Inherit;False;Property;_WhitNoiseLerpTex1;Whit Noise Lerp Tex 1;18;0;Create;True;0;0;False;0;False;0;0;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.FunctionNode;167;-221.2828,381.0264;Inherit;False;TextureBlend;-1;;239;24e89ccf6f531df44bc7fec5ad48af13;0;8;53;FLOAT;0;False;46;FLOAT;0;False;47;FLOAT;0;False;22;FLOAT3;0,0,0;False;23;FLOAT3;0,0,0;False;26;FLOAT3;0,0,0;False;27;FLOAT3;0,0,0;False;20;FLOAT3;0,0,0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.FunctionNode;168;-471.2651,-528;Inherit;False;TextureBlend;-1;;235;24e89ccf6f531df44bc7fec5ad48af13;0;8;53;FLOAT;0;False;46;FLOAT;0;False;47;FLOAT;0;False;22;COLOR;0,0,0,0;False;23;COLOR;0,0,0,0;False;26;COLOR;0,0,0,0;False;27;COLOR;0,0,0,0;False;20;FLOAT3;0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.StandardSurfaceOutputNode;0;1024.679,465.7168;Float;False;True;-1;6;ASEMaterialInspector;0;0;Standard;TerrainMixer;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;Back;0;False;-1;0;False;-1;False;0;False;-1;0;False;-1;False;0;Opaque;0.5;True;True;0;False;Opaque;;Geometry;ForwardOnly;14;all;True;True;True;True;0;False;-1;False;0;False;-1;255;False;-1;255;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;-1;True;2;15;10;25;False;0.5;True;0;0;False;-1;0;False;-1;0;0;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;0;0,0,0,0;VertexOffset;True;False;Cylindrical;False;Relative;0;;-1;-1;-1;-1;0;False;0;0;False;-1;-1;0;False;-1;0;0;0;False;0.1;False;-1;0;False;-1;False;16;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;2;FLOAT3;0,0,0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT3;0,0,0;False;7;FLOAT3;0,0,0;False;8;FLOAT;0;False;9;FLOAT;0;False;10;FLOAT;0;False;13;FLOAT3;0,0,0;False;11;FLOAT3;0,0,0;False;12;FLOAT3;0,0,0;False;14;FLOAT4;0,0,0,0;False;15;FLOAT3;0,0,0;False;0
...
ASEEND*/
//CHKSM=CD3033234E317F066EB31B1B91906FC748D585E1